...
var TGE = function(environment, config, commands)
{
	...
	command = function(cmdArgs)
	{
		...
		// pass all the data to the listeners only if the command was defined
		/*if (Object.keys(this.commands).indexOf(cmd) > -1)
			for (i=0; i < this.cmdListeners.length; i++)
				this.cmdListeners[i](cmd, args);*/
		if (this.commands.hasOwnProperty(cmd))
		{
			if (this.commands[cmd].hasOwnProperty('exec'))
	...
	/*this.addCmdListener = function(fn)
	{
		this.cmdListeners.push(fn);
		console.log(this, this.cmdListeners);
	}.bind(this);*/
